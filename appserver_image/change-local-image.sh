...
function showUsage()
{
    echo "PURPOSE: Save a LOCAL image to your docker image collection with a new TAG"
    echo "         and with the LOCAL designation removed."
    echo
...
CMD_TAG="docker tag local/${IMAGE_NAME}:${CURRENT_TAG_NAME} ${IMAGE_NAME}:${NEW_TAG_NAME}"
echo $CMD_TAG
eval $CMD_TAG
echo
